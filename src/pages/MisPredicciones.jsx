  ...
  return (
    <div>
      ...
      <div style={styles.resumenGrid}>
        ...
        <div style={styles.resumenCard}>
          <div style={{ ...styles.resumenNum, color: '#22c55e' }}>{exactos}</div>
          <div style={styles.resumenLabel}>Resultados exactos</div>
        </div>
        <div style={styles.resumenCard}>
          <div style={{ ...styles.resumenNum, color: 'var(--celeste-dark)' }}>{ganadores}</div>
      ...
      {cargando ? (
        ...
        <div style={styles.vacio}>
          ...
          <div style={{ fontWeight: 600 }}>No cargaste ningún prode todavía</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {predicciones.map(p => (
            <div key={p._id} style={styles.filaPrediccion}>
              <div style={styles.filaEquipos}>
                <span style={styles.equipoNombre}>{p.partido?.local}</span>
                <span style={styles.marcadorPrediccion}>
                  {p.golesLocal} - {p.golesVisitante}
                </span>
                <span style={styles.equipoNombre}>{p.partido?.visitante}</span>
              </div>

              {p.puntos !== undefined && p.puntos !== null ? (
                <div style={styles.resultadoFila}>
                  ...
                  <span className={`badge-puntos ${
                    p.puntos === 3 ? 'badge-exacto' : p.puntos === 1 ? 'badge-ganador' : 'badge-error'
                  }`}>
                    {p.puntos === 3 ? '⭐ 3 pts' : p.puntos === 1 ? '✓ 1 pt' : '0 pts'}
                  </span>
                </div>
              ) : (
                <span style={{ fontSize: 12, color: 'var(--texto-secundario)' }}>Pendiente</span>
              )}
            </div>
          ))}
        </div>
...
const styles = {
  ...
  resumenLabel: { fontSize: 12, color: 'var(--texto-secundario)', marginTop: 4 },
  vacio: {
    textAlign: 'center',
    padding: '48px 20px',
    background: 'var(--blanco)',
