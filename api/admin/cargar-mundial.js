...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const partido of PARTIDOS_MUNDIAL) {
      const banderaLocal =
        SELECCIONES[partido.local] || `https://flagcdn.com/w80/un.png`;
      const banderaVisitante =
        SELECCIONES[partido.visitante] || `https://flagcdn.com/w80/un.png`;

      await db.collection("partidos").updateOne(
        { local: partido.local, visitante: partido.visitante, esMundial: true },
        {
          $set: {
            local: partido.local,
