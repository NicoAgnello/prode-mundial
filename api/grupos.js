...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const grupo = await db.collection('grupos').findOne({ _id: usuario.grupoId })
    ...
    if (req.method === 'POST') {
      ...
      if (usuario.grupoId) {
        const grupoActual = await db.collection('grupos').findOne({ _id: usuario.grupoId })
      ...
      if (!grupo) {
        return res.status(404).json({ error: 'Código inválido — verificá que esté bien escrito' })
      }
