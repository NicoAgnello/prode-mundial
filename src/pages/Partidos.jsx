...
function CardPartido({ partido, prediccion, onGuardar, puedeProde }) {
  ...
  const enJuego = partido.estado === '1H' || partido.estado === '2H' || partido.estado === 'HT'
  ...
  const badgePuntos = () => {
    ...
    if (prediccion.puntos === 3) return <span className="badge-puntos badge-exacto">⭐ 3 pts</span>
    if (prediccion.puntos === 1) return <span className="badge-puntos badge-ganador">✓ 1 pt</span>
    ...
    <div style={styles.card}>
      ...
      <div style={styles.equipos}>
        <div style={styles.equipo}>
          <span style={styles.bandera}>{partido.banderaLocal}</span>
        ...
        <div style={{ ...styles.equipo, alignItems: 'flex-end' }}>
          <span style={styles.bandera}>{partido.banderaVisitante}</span>
      ...
      <div style={styles.pronostico}>
        {yaJugo ? (
          <div style={styles.resultadoProde}>
            {prediccion ? (
              <>
                <span style={styles.prodeLabel}>Tu prode: {prediccion.golesLocal} - {prediccion.golesVisitante}</span>
                {badgePuntos()}
              </>
            ) : (
              <span style={{ color: 'var(--texto-secundario)', fontSize: 13 }}>No cargaste prode para este partido</span>
            )}
          </div>
        ) : puedeProde ? (
          <div style={styles.inputArea}>
            ...
            <div style={styles.inputRow}>
              ...
              >
                {guardando ? '...' : guardado ? '✓ Guardado' : prediccion ? 'Actualizar' : 'Guardar'}
              </button>
            </div>
          </div>
        ) : (
...
const styles = {
  ...
  bandera: { fontSize: 28 },
  nombreEquipo: { fontSize: 13, fontWeight: 600, color: 'var(--texto-principal)' },
  ...
  guion: { fontFamily: 'var(--font-display)', fontSize: 28, color: 'var(--texto-secundario)' },
  vs: {
    fontFamily: 'var(--font-display)',
    fontSize: 20,
    color: 'var(--texto-secundario)',
  ...
  prodeLabel: { fontSize: 12, color: 'var(--texto-secundario)', marginBottom: 6, display: 'block' },
